...
class QueryClassifier:
    ...
    def router(self, query):
        ...
        routing_llm = ChatGroq(model=GROQ_MODEL, temperature=0,
        ...
        while True:
            try:
                ...
                if isinstance(routing, dict) and "source" in routing and routing["source"] in ["vectorstore",
                                                                                               "chatbot"]:
                    return routing
                else:
...
class DocumentGrader:
    def __init__(self, model=GROQ_MODEL, temperature=0.0):  # mistral:7b-instruct-q8_0, atla/selene-mini:fp_16
        self.grader = ChatGroq(model=model, temperature=temperature).with_structured_output(ScoringSchema)
...
class SimpleChatBot:
    def __init__(self, model=GROQ_MODEL, temperature=0.7):
        self.chatbot = ChatGroq(model=model, temperature=temperature)
...
class FoodChatResponseEvaluator:
    def __init__(self):
        self.question_generator = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(QuestionList)
        self.question_responder = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(
    ...
    def generate_questions(self, query):
        ...
        question_list = self.question_generator.invoke([SystemMessage(
            content=SIMPLE_QUESTIONS_GENERATOR_SYSTEM_INSTRUCTIONS)] + question)  # if the initial query is too simple then just use the initial query directly
    ...
    def evaluate(self, query, response):
        ...
        question = [HumanMessage(
            content=SIMPLE_QUESTIONS_RESPONDER_USER_INSTRUCTIONS.format(question_list=question_list,
                                                                        response=response))]
        eval_result = self.question_responder.invoke(
            [SystemMessage(content=SIMPLE_QUESTIONS_RESPONDER_SYSTEM_INSTRUCTIONS)] + question)
...
class QueryRewriter:
    def __init__(self):
        self.query_rewriter = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(QueryRewriterSchema)
...
class FeedBackRewriter:
    def __init__(self):
        self.feedback_rewriter = ChatGroq(model="llama3.2:latest", temperature=0.0, top_k=1, top_p=0)

    def rewrite(self, feedback, response):
        question = [
            HumanMessage(content=FEEDBACK_REWRITER_USER_INSTRUCTIONS.format(response=response, feedback=feedback))]
...
class QueryReconciler:
    def __init__(self):
        self.query_item_extractor = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(
            QueryItemExtractorSchema)
        self.query_reconciler = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(
...
class RAGReadyPreparator:
    ...
    def __init__(self):
        self.query_checker = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(QueryCheckerSchema)
        self.user_profile_checker = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(
            UserProfileCheckerSchema)
        self.user_info_collector = ChatGroq(model=GROQ_MODEL, temperature=0.0)
        self.query_reformulator = ChatGroq(model=GROQ_MODEL, temperature=0.0).with_structured_output(
    ...
    def collect_user_info(self, query, suggestions, user_info):
        ...
        for suggestion in suggestions:
            ...
            response = self.user_info_collector.invoke([
                                                           SystemMessage(
                                                               content=USER_INFO_COLLECTOR_SYSTEM_ISNTRUCTIONS)] + question + conversation_history)
    ...
    def query_check(self, query, user_profile):
        ...
        if response == "NO":
            ...
            if user_profile_check['response'] == 'NO':
                print(Fore.RED + "List of Subject to collect information: " + str(
                    user_profile_check['suggestions']) + Style.RESET_ALL)
                collected_info = yield from self.collect_user_info(query, user_profile_check['suggestions'],
                                                                   user_profile)
